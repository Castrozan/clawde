        ...
        { config, ... }:
        {
          options = {
            ...
            sidecarProcessSpecificationsFor = lib.mkOption {
              ...
              default = _: [ ];
              description = "Function: { name, agent, workspaceDirectory, launchConfigPath } -> headless processes this adapter needs running beside the agent, each { name, command, process_match_pattern }. Used when the harness carries no inbound transport of its own and the channel has to be bridged by a separate process. The supervisor owns these directly rather than giving them a multiplexer window, so the agent's own window stays the single entrypoint a human opens, and their output goes to a log file instead of a pane. launchConfigPath names the agent's deploy-time launch config, which carries the per-harness one-shot turn commands the bridge needs so it can follow the agent across a runtime harness failover. process_match_pattern must identify exactly this agent's process in a `pgrep -f` over the whole machine, so it has to carry a delimiter after the agent name or it matches every agent whose name starts with this one.";
            };
