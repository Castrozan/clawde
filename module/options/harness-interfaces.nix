        ...
        { name, config, ... }:
        {
          options = {
            ...
            buildOneShotTurnCommandFor = lib.mkOption {
              ...
              default = null;
              description = "Function: { name, agent, workspaceDirectory, instructionsFile } -> the shell command that runs one turn of a conversation and exits, taking its prompt from $CLAWDE_CHANNEL_PROMPT and continuing the previous turn when $CLAWDE_CHANNEL_SESSION_CONTINUATION is non-empty. A completed turn exits zero and writes the final assistant JSON envelope from module/lib/channel-reply-schema.json to $CLAWDE_CHANNEL_REPLY_FILE. A completed turn without assistant output writes an empty file. The channel adapter validates reply intent; invalid envelopes are discarded without counting the harness as failed. Failed or incomplete turns exit nonzero. Missing result files are failures. Diagnostics, reasoning and tool output never belong in the reply file. Null means the harness has no headless mode, so channel adapters that drive it from a sidecar process cannot carry it.";
            };
