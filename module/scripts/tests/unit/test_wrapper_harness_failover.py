...
import pytest
from harness_productivity_delivery import baseline_next_delivery
        ...
        json.dumps(
            {
                ...
                "harness_runtime_profiles": {
                    "opencode": OPENCODE_PROFILE_MAPPING,
...
def test_three_crashes_after_delivery_fail_over_to_the_next_harness(
    ...
    def fake_run_launch_command_once(
        launch_command, _heartbeat_driver_argv, _tmux_target, **_kwargs
    ):
        launch_commands_run.append(launch_command)
        if launch_command == "codex" or len(launch_commands_run) > 3:
            raise StopSupervising()
        begin_harness_productivity_record(record_path, "opencode")
        baseline_next_delivery(record_path, "harness-owned-session", None)
    ...
    assert launch_commands_run == ["opencode", "opencode", "opencode", "codex"]
